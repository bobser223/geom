...
*Означення 1* Вершина $p_j$ вважається _видимою_ з вершини $p_i$, якщо не знайдеться жодного такого відрізка (сермента) $s_k$, що перетне сегмент (відрізок) $[p_j, p_i]$, в тривіальному випадку коли $s_k$ = $[p_j, p_i]$ точка $p_j$ - видима.

=== *2.1.1 постановка задачі побудування графа видимості*
Нехай є множина серментів - ребер перешкод $SS$, а також множин точок - вершин цих ребер $PP$, для кожної вершини $p_i in PP$ потрібно знайти всі такі вершини $p_j in PP, i != j$, що _видимі (див. Означення 1)_, щойно ми пропробимо такий пошук для $forall i = 1,...,|P|$ ми отрмаємо граф видимості заданий списком суміжності $p_i ->$ {список видимих для неї вершин}
...
Ідія Лі, полягає в використанні замітаючого променя, де подією є вершина (точка) сегмента, а статус - впорядкований список сегментів
...
== Покрокова демонстарія на приватному випадку
